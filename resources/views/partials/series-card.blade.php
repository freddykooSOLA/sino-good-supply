@php
    ...
    $detailUrl = locale_url('series/'.$series->localizedSlug());
@endphp
...
<article class="group">
    ...
    >
        @if(! empty($images))
            ...
            @if($hasSlider)
                <button type="button" class="product-slider__nav left-2" data-slider-prev aria-label="Previous image">
                ...
                <button type="button" class="product-slider__nav right-2" data-slider-next aria-label="Next image">
    ...
    <a href="{{ $detailUrl }}" class="block pt-4">
        @if($series->category)
            <div class="text-xs uppercase tracking-wider text-gold-accent/80">{{ $series->category->localizedName() }}</div>
        @endif
        <h3 class="mt-1 text-base font-medium text-white transition group-hover:text-gold-accent">{{ $series->localizedName() }}</h3>
        @if($series->localizedIntro())
            <p class="mt-2 line-clamp-2 text-sm text-white/55">{{ $series->localizedIntro() }}</p>
        @endif
